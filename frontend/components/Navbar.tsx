...
export default function Navbar() {
  ...
  // Helper: Desktop Nav Item with HOVER
    ...
    <header className="fixed top-0 w-full z-[1000] bg-white/95 dark:bg-[#020817]/95 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800">
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 md:gap-3 z-[1001]">
          ...
          <button 
            className="md:hidden p-2 text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition z-[1002]" 
    ...
    <AnimatePresence>
        {mobileMenu && (
            <motion.div 
                ...
                className="fixed inset-0 top-16 bg-white dark:bg-[#020817] z-[999] overflow-y-auto md:hidden"
            >
                        ...
                        {user ? (
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <button onClick={() => { logout(); setMobileMenu(false); }} className="text-xs text-red-500 font-bold flex items-center gap-1 mt-1">
                                    Log Out <LogOut size={12}/>
                                  </button>
                                </div>
                            </div>
                        ) : (
                            <Link href="/login" onClick={() => setMobileMenu(false)} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold text-center flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20">
                    ...
                    <div className="flex-1 space-y-1">
                        ...
                        {navLinks.map((nav, i) => (
                          <div key={i} className="border-b border-slate-100 dark:border-slate-800">
                            ...
                            >
                                {nav.name}
                                <ChevronDown size={20} className={`transition-transform duration-300 ${mobileActive === nav.name ? "rotate-180 text-emerald-500" : ""}`}/>
                            </button>
                            
                            <AnimatePresence>
                                {mobileActive === nav.name && (
                                    <motion.div 
                        ...
                        <div className="border-b border-slate-100 dark:border-slate-800 py-4">
                          <Link href="/news" onClick={() => setMobileMenu(false)} className="flex items-center justify-between text-lg font-bold text-blue-600 dark:text-blue-400">
                            News Hub 📰 <ChevronRight size={20}/>
                          </Link>
                        </div>
                        <div className="border-b border-slate-100 dark:border-slate-800 py-4">
                          <Link href="/chill-zone" onClick={() => setMobileMenu(false)} className="flex items-center justify-between text-lg font-bold text-pink-600 dark:text-pink-400">
                            Chill Zone 🍿 <ChevronRight size={20}/>
                          </Link>
                        </div>
                    </div>
                </div>
