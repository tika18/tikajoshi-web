...
function MarketStatus() {
  ...
  if (day === 5 || day === 6) {
    ...
    const mins = openMinutes - minutesNow;
    statusStr = `Market Opens in ${Math.floor(mins / 60)}h ${mins % 60}m`;
    isClosed = true;
  } else if (minutesNow >= closeMinutes) {
    statusStr = `Market Closed for Today`;
    isClosed = true;
  } else {
    const mins = closeMinutes - minutesNow;
    statusStr = `Market is OPEN - Closes in ${Math.floor(mins / 60)}h ${mins % 60}m`;
  }
...
export default function MarketPage() {
  ...
  const fetchData = useCallback(async (showRefreshIndicator = false) => {
    ...
    try {
      const res = await fetch("/api/nepse", { cache: "no-store" });
      const json: NepseData = await res.json();
      setData(json);
      setSourceUpdated(json.sourceUpdatedText || "");
      setLastUpdated(
        new Date(json.timestamp || Date.now()).toLocaleTimeString("en-NP", {
          hour: "2-digit",
          minute: "2-digit",
          second: "2-digit",
        })
  ...
  // Initial fetch + near realtime refresh every 20 seconds
  useEffect(() => {
    fetchData();
    const interval = setInterval(() => fetchData(false), 20 * 1000);
