...
import { 
  ...
  UserSquare, Calculator, Zap, Globe, FileDown, Layers, Lock, RefreshCw, FileText as BlogIcon
} from "lucide-react";
...
export default function ToolsPage() {
  ...
  // Fetch blogs related to this page
  useEffect(() => {
    fetch("/api/blogs?targetPage=tools")
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          setBlogs(data.posts || []);
      ...
      .catch(err => console.error("Error fetching tools page blogs:", err));
  }, []);
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200">
      ...
      <div className="max-w-7xl mx-auto px-6 py-24">
        ...
        <div className="text-center mb-16 max-w-2xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-6 animate-in fade-in duration-500">
        ...
        {blogs.length > 0 && (
          <div className="border-t border-slate-800 pt-16">
            <div className="flex items-center gap-3 mb-10">
              <BlogIcon className="text-emerald-500" />
              <h2 className="text-2xl font-black text-white uppercase tracking-wider">
                Tools Guides & Documentation 📄
              </h2>
            </div>
            <div className="grid md:grid-cols-3 gap-6">
              {blogs.map((blog) => (
                <Link
                  key={blog._id}
                  href={`/blog/${blog.slug}`}
                  className="group bg-slate-900/60 border border-slate-800 hover:border-emerald-500/30 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1"
                >
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="text-base font-bold text-white mb-2 group-hover:text-emerald-400 transition-colors line-clamp-2 leading-snug">
                      {blog.title}
                    </h3>
